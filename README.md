...
## System Requirements
One of our goals in developing cosipy is to make it easily accesible to all users. **All of the data challenges in DC2 should be doable on a laptop with at least 16 GB of memory**. The biggest memory limitation will likely come from binning the data. In general, as analyses become more sophisticated, it is inevitable that they may require more time and memory. We are still working on optimizing the code, and so if you are running into memory issues, please let us know. 
