...
Every analyses in cosipy is based on the following likelihood function:
...
The probability of observing a given number of counts based on the expectation is described by a Poisson distribution. In other words, we are assuming that the photons are totally independent from each other

Although there are multiple ways to use the likelihood to perform inference analysis, so far we have only performs a maximum likelihood estimations (MLE). We goal is to obtain the best estimates for the real values of the sky model parameters given the data, and those correspond to the parameters the maximize the likelihood --i.e. those that maximize the probability of obtaining our data sample. The corresponding equation is:
...
In principle, for each set of sky model parameters, we could run the MEGAlib simulations and see how many event we obtain in average, which corresponds to the expected counts. This is not feasiable since we need to run this multiple times while we sample the parameters space. Instead, we build a detector response matrix that encodes the information that we need. 

In order to understand the mechanics, let's forget for a moment about imaging and assume the only measured value is the energy. For this case, we simulate multiple photons at various _real_ energies ($E$) and record how many we detect in total ($N_{det}$), and how their _measured_ energies are distributed ($E'$). Since we know the the events per unit area (flux) used in the simulations, we can compute the effective area as:

$$A_{eff} = \frac{N_{det}}{flux}$$

The effective area is a function of the energy, so we choose multiple values. The effective area of each of this sampled points, distributed based on the _measured_ energies observed in the simulations, is the detector response matrix:
...
- (l',b'): The direction of the scattered gamma after the first interation. Typically in galactic coordinates.
- $\phi$: The polar scattering angle. Although we don't know the direction of the incoming gamma ray, we do know scattering angle due to kinematics.

<img src="figures/CDS_detector.png" alt="" width="250"/>

This is key to perform imaging, since the photon from a source form clearly identified cone in the CDS. This is what allows us to disentangle the different source in our field of view at any given time! The following shows an example for two sources: if we only had (l',b'), we could not resolve the different sources:

<img src="figures/CDS_two_sources.png" alt="" width="500"/>

### The detector response: full version

As we saw in the previous section, in order to do spectral *and* imaging we need to bin the data into both the measured energy and the full CDS (four dimensions). Respetivly, the total expected counts and their distribution depend on the real photon energy and the source location (three dimensions). The full response is then a 7-dimensional matrix, composed of both physical and measured axes:

<img src="figures/full_drm.png" alt="" width="250"/>

Although this might look more complicated, the mechanics are exactly the same a for the simplified spectral analysis with a 2D response that we saw in the [previous section](#anchor_rsp_intro). The expected number of counts is still a matrix multiplication. The projection onto the physical axes is still the effective area. A slice for a given combination of energy, measured energy and direction (red dot) looks like this in the Compton Data Space: 
...
- The DataIO module perform the binning of the data ($n_i$). 
- The Spacecraft File module keep track of the spacecraft orientation, so we can transform galactic coordinate to detector coordinate.
- The Detector Response module reads the response matrix obtain from MEGAlib and produces the expected signal counts.
...
The likelihood is the maximized by other modules, which have different goals:
...
- Image deconvolution: the sky model is discretized, effective having one free parameter for every sky pixel and energy bin. The main advantage is that is it not model dependent; the main disadvantage is that it is hard to estimate errors due to the high correlation between the different parameters.

Cosipy does not currently includes polarization analysis, that will wait for the next data challenge.

Although cosipy doesn't have a source injector (simulator) yet, it is relative easy to do given the expected counts. It would be a good excersice/challenge.

Internally, all modules handle the data using the objects:

- [histpy.Histogram](https://histpy.readthedocs.io/en/latest/): labeled axes in a matrix, support sparse matrices, performs projections and slice operations, etc.
...
The Multi-Mission Maximum Likelihood framework ([3ML](https://threeml.readthedocs.io/en/stable/)) is a common interface to perform a likelihood-based analysis across multiple instruments. Since all instrument observe the same source, their likelihoods for a common source model can be simple multiplied to obtain the global likelihood:
...
All 3ML need is a plugin for each instrument that accepts a common model in a predetermined format ([astromodels](https://astromodels.readthedocs.io/en/latest/)), convolves it with its particular instrument response, and return a likelihood. This is precisely what COSILike does.
...
- The response and data are easier to visualize and manipulate, thanks to using:
    - mhealpy: using the HEALPix standard (sphere grid)
    - histpy: labeled axes in a matrix, support sparse matrices, performs projections and slice operations, etc.
...
Cosipy is under active development, in prepartion for the COSI launch scheduled in 2027. Some of the following steps are:

- Clean thing up a little more: improve the documentation, standariza the API across all modules, develop unit tests until we have full coverage
- Develop the polarization analysis. The detector response need a new axis: the photon polarization angle
- The scalability issue. Currently we have a coarse detector reponse and no polarization, and the code is still relatively slow. This will not be sustainble when we use a binning appropiate for COSI. Some options are:
    - Further code optimization. We haven't spend a lot of time on this, there might still be low hanging fruits.
    - Decouple the response matrix from data binning. Although  currently we do a direct matrix multiplication, we can effectively perform this operation in clever ways. (Note from Israel: an unbinned analysis is an option!)
