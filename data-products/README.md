...
**Important Note:** Combining the data and binning the data can be memory intensive. If you are running into memory limitations at these steps, then the solution is to first bin each individual component in chuncks (as described in the dataIO example), and then sum the binned data histograms (as described [here](https://histpy.readthedocs.io/en/latest/tutorials/quick-start.html#Filling)). Alternatively, we have also provided binned data products for some of the larger files (total background, 511, and Al26), which can be loaded directly. In each respective case, the binning matches the response, and has a time bin size of 7202.125 seconds.  
